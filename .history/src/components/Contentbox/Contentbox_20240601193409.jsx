...
const Contentbox = () => {
  ...
  useEffect(() => {
    const color = qrColor.slice(1);
    const bgColor = qrBgColor.slice(1);
    const data = textval ? textval : "Welcome To QRBuilder";

    setQrData(
      `https://api.qrserver.com/v1/create-qr-code/?size=${qrSize}x${qrSize}&margin=${margin}&color=${color}&bgcolor=${bgColor}&format=${formatType}&data=${data}`
    );
  }, [textval, qrSize, margin, qrColor, qrBgColor, formatType]);
          ...
          <div className="displaybox">
            ...
            <div className="qrsettings">
              <div className="dimension">
                ...
                />
                <p>
                  Your Dimension : {qrSize}x{qrSize}
                </p>
              </div>
              <h1>Color</h1>
              <input
              ...
              <input
                ...
                name="qrzone"
                id="qrzone"
                placeholder="Enter qrzone value"
                value={qzone}
                onChange={(e) => {
                  setQzone(e.target.value);
                }}
              />
            </div>

            <div className="qrbuttons">
              <button className="qrbutton" onClick={() => handleDownload("png")}>
                <img
                  width="48"
                  height="48"
                  src="https://img.icons8.com/fluency/48/download.png"
              ...
              <button className="qrbutton" onClick={() => handleDownload("jpeg")}>
                <img
                  width="48"
                  height="48"
                  src="https://img.icons8.com/fluency/48/download.png"
